...
describe Puppet::Type.type(:package).provider(:npm) do
  before :each do
    ...
    @provider.class.stubs(:optional_commands).with(:npm).returns "/usr/bin/npm"
    @provider.class.stubs(:command).with(:npm).returns "/usr/bin/npm"
  end
  ...
  describe "when npm packages are installed globally" do
    ...
    it "should return a list of npm packages installed globally" do
      @provider.class.expects(:execute).with(['/usr/bin/npm', 'list', '--json', '--global'], anything).returns(my_fixture_read('npm_global'))
    ...
    it "should log and return no packages if JSON isn't output" do
      @provider.class.expects(:execute).with(['/usr/bin/npm', 'list', '--json', '--global'], anything).returns("failure!")
